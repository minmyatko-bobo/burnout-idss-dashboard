...
Kept separate from dashboard.py so it can be unit-tested without launching
Streamlit: RBAC filtering, summaries, intervention mapping, and PDF export.
"""
...
WORKFORCE_PATH = ROOT/"data"/"sehim_workforce.json"
...
def load_workforce(path: Path = WORKFORCE_PATH) -> list[dict]:
    # Prefer the JSON file if present; otherwise use the embedded copy so the
    # deployed app always has data even if the data/ folder was not uploaded.
    if path.exists():
        return json.loads(path.read_text())
    from workforce_data import WORKFORCE
    return WORKFORCE


def filter_by_role(emps: list[dict], role: str, department: str | None) -> list[dict]:
    if role == "HR_Admin":
        return emps
    if role == "HR_Manager":
        if not department:
            return []
        return [e for e in emps if e["department"] == department]
    return []


def to_frame(emps: list[dict]) -> pd.DataFrame:
    if not emps:
...
def interventions_for(level: str) -> list[str]:
    return INTERVENTIONS.get(level, [])


def build_employee_pdf(emp: dict) -> bytes:
    """Render a one-page HR risk report (reportlab) -> PDF bytes."""
    ...
    el = [Paragraph("Employee Burnout Risk Report", h),
          ...
          Spacer(1, 8)]
    info = Table([
        ["Employee ID", emp["employeeId"], "Risk level", emp["riskLevel"]],
        ["Department", emp["department"], "CBRI score", f"{emp['cbri']:.3f}"],
        ["Job title", emp["jobTitle"], "", ""],
    ], colWidths=[70, 150, 70, 110])
    ...
    scores.setStyle(TableStyle([
        ("FONTSIZE",(0,0),(-1,0),9),("TEXTCOLOR",(0,0),(-1,0),colors.grey),
    ...
    el.append(Paragraph("Why this employee is flagged", body))
    for f in emp["topFactors"]:
        el.append(Paragraph(f"&bull; {f['factor']} ({f['direction']} risk)", body))
    el += [Spacer(1, 8), Paragraph("Explanation", body),
           Paragraph(emp["explanationNarrative"], body), Spacer(1, 8),
           Paragraph("Suggested intervention actions", body)]
